...
namespace SilkFlo.Data.Persistence.Repositories.Business
{
  public class IdeaStageStatusRepository : IIdeaStageStatusRepository
  {
    ...
    {
      DataSet dataSetAsync = await UnitOfWork.GetDataSetAsync();
      return (IEnumerable<IdeaStageStatus>) (await dataSetAsync.BusinessIdeaStageStatuses.ToListAsync<IdeaStageStatus>()).OrderBy<IdeaStageStatus, string>((Func<IdeaStageStatus, string>) (m => m.IdeaStageId)).ThenBy<IdeaStageStatus, DateTime>((Func<IdeaStageStatus, DateTime>) (m => m.Date));
    }
