...
namespace SilkFlo.Data.Persistence.Repositories.Business
{
  public class DocumentRepository : IDocumentRepository
  {
    ...
    {
      DataSet dataSetAsync = await UnitOfWork.GetDataSetAsync();
      return (IEnumerable<Document>) (await dataSetAsync.BusinessDocuments.ToListAsync<Document>()).OrderBy<Document, string>((Func<Document, string>) (m => m.ClientId)).ThenBy<Document, string>((Func<Document, string>) (m => m.Filename)).ThenBy<Document, string>((Func<Document, string>) (m => m.Text));
    }
