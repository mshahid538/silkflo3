...
namespace SilkFlo.Data.Persistence.Repositories.Business
{
    public class ClientRepository : IClientRepository
    {
        ...
        {
            DataSet dataSetAsync = await UnitOfWork.GetDataSetAsync();
            return (IEnumerable<Client>) (await dataSetAsync.BusinessClients.ToListAsync<Client>()).OrderBy<Client, string>((Func<Client, string>)(m => m.Name));
        }
        ...
        public void GetClientFor(IEnumerable<Core.Domain.Business.Application> applications) => this.GetClientForAsync(applications).RunSynchronously();

        public async Task GetClientForAsync(IEnumerable<Core.Domain.Business.Application> applications)
        {
        ...
        public void GetClientFor(Core.Domain.Business.Application application) => this.GetClientForAsync(application).RunSynchronously();

        public async Task GetClientForAsync(Core.Domain.Business.Application application)
        {
            if (application == null)
                ;
            else
            {
                Core.Domain.Business.Application application1 = application;
                DataSet dataSet = await UnitOfWork.GetDataSetAsync();
                application1.Client = dataSet.BusinessClients.SingleOrDefault<Client>((Func<Client, bool>)(x => x.Id == application.ClientId));
                application1 = (Core.Domain.Business.Application)null;
                dataSet = (DataSet)null;
            }
        }
