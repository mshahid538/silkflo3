...
namespace SilkFlo.Data.Persistence.Repositories.Business
{
  public class IdeaStageRepository : IIdeaStageRepository
  {
    ...
    {
      DataSet dataSetAsync = await UnitOfWork.GetDataSetAsync();
      return (IEnumerable<IdeaStage>) (await dataSetAsync.BusinessIdeaStages.ToListAsync<IdeaStage>()).OrderBy<IdeaStage, string>((Func<IdeaStage, string>) (m => m.IdeaId)).ThenBy<IdeaStage, DateTime>((Func<IdeaStage, DateTime>) (m => m.DateStartEstimate)).ThenBy<IdeaStage, string>((Func<IdeaStage, string>) (m => m.StageId));
    }
