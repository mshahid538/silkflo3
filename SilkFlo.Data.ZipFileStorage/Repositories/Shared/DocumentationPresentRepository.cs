...
namespace SilkFlo.Data.Persistence.Repositories.Shared
{
  public class DocumentationPresentRepository : IDocumentationPresentRepository
  {
    ...
    {
      DataSet dataSetAsync = await UnitOfWork.GetDataSetAsync();
      return (IEnumerable<DocumentationPresent>) (await dataSetAsync.SharedDocumentationPresents.ToListAsync<DocumentationPresent>()).OrderByDescending<DocumentationPresent, Decimal>((Func<DocumentationPresent, Decimal>) (m => m.Weighting)).ThenBy<DocumentationPresent, string>((Func<DocumentationPresent, string>) (m => m.Name));
    }
