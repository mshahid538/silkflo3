...
namespace SilkFlo.Data.Persistence.Repositories.Shared
{
  public class InputDataStructureRepository : IInputDataStructureRepository
  {
    ...
    {
      DataSet dataSetAsync = await UnitOfWork.GetDataSetAsync();
      return (IEnumerable<InputDataStructure>) (await dataSetAsync.SharedInputDataStructures.ToListAsync<InputDataStructure>()).OrderByDescending<InputDataStructure, Decimal>((Func<InputDataStructure, Decimal>) (m => m.Weighting)).ThenBy<InputDataStructure, string>((Func<InputDataStructure, string>) (m => m.Name));
    }
