...
namespace SilkFlo.Data.Persistence.Repositories.Shared
{
  public class IdeaAuthorisationRepository : IIdeaAuthorisationRepository
  {
    ...
    {
      DataSet dataSetAsync = await UnitOfWork.GetDataSetAsync();
      return (IEnumerable<IdeaAuthorisation>) (await dataSetAsync.SharedIdeaAuthorisations.ToListAsync<IdeaAuthorisation>()).OrderBy<IdeaAuthorisation, int>((Func<IdeaAuthorisation, int>) (m => m.Sort)).ThenBy<IdeaAuthorisation, string>((Func<IdeaAuthorisation, string>) (m => m.Name));
    }
