...
namespace SilkFlo.Data.Persistence.Repositories.Shared
{
  public class IdeaStatusRepository : IIdeaStatusRepository
  {
    ...
    {
      DataSet dataSetAsync = await UnitOfWork.GetDataSetAsync();
      return (IEnumerable<IdeaStatus>) (await dataSetAsync.SharedIdeaStatuses.ToListAsync<IdeaStatus>()).OrderBy<IdeaStatus, int>((Func<IdeaStatus, int>) (m => m.Sort)).ThenBy<IdeaStatus, string>((Func<IdeaStatus, string>) (m => m.Name));
    }
