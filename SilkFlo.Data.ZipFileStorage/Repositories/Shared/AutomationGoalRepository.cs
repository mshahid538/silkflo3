...
namespace SilkFlo.Data.Persistence.Repositories.Shared
{
  public class AutomationGoalRepository : IAutomationGoalRepository
  {
    ...
    {
      DataSet dataSetAsync = await UnitOfWork.GetDataSetAsync();
      return (IEnumerable<AutomationGoal>) (await dataSetAsync.SharedAutomationGoals.ToListAsync<AutomationGoal>()).OrderBy<AutomationGoal, int>((Func<AutomationGoal, int>) (m => m.Sort)).ThenBy<AutomationGoal, string>((Func<AutomationGoal, string>) (m => m.Name));
    }
