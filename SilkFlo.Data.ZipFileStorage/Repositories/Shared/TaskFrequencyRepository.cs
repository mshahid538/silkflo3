...
namespace SilkFlo.Data.Persistence.Repositories.Shared
{
  public class TaskFrequencyRepository : ITaskFrequencyRepository
  {
    ...
    {
      DataSet dataSetAsync = await UnitOfWork.GetDataSetAsync();
      return (IEnumerable<TaskFrequency>) (await dataSetAsync.SharedTaskFrequencies.ToListAsync<TaskFrequency>()).OrderBy<TaskFrequency, int>((Func<TaskFrequency, int>) (m => m.Sort)).ThenBy<TaskFrequency, string>((Func<TaskFrequency, string>) (m => m.Name));
    }
