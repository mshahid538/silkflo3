...
namespace SilkFlo.Data.Persistence.Repositories.Shared
{
  public class AverageNumberOfStepRepository : IAverageNumberOfStepRepository
  {
    ...
    {
      DataSet dataSetAsync = await UnitOfWork.GetDataSetAsync();
      return (IEnumerable<AverageNumberOfStep>) (await dataSetAsync.SharedAverageNumberOfSteps.ToListAsync<AverageNumberOfStep>()).OrderByDescending<AverageNumberOfStep, Decimal>((Func<AverageNumberOfStep, Decimal>) (m => m.Weighting)).ThenBy<AverageNumberOfStep, string>((Func<AverageNumberOfStep, string>) (m => m.Name));
    }
