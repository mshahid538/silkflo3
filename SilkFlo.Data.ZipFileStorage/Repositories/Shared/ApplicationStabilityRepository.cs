...
namespace SilkFlo.Data.Persistence.Repositories.Shared
{
  public class ApplicationStabilityRepository : IApplicationStabilityRepository
  {
    ...
    {
      DataSet dataSetAsync = await UnitOfWork.GetDataSetAsync();
      return (IEnumerable<ApplicationStability>) (await dataSetAsync.SharedApplicationStabilities.ToListAsync<ApplicationStability>()).OrderByDescending<ApplicationStability, Decimal>((Func<ApplicationStability, Decimal>) (m => m.Weighting)).ThenBy<ApplicationStability, string>((Func<ApplicationStability, string>) (m => m.Name));
    }
