...
namespace SilkFlo.Data.Persistence.Repositories.Shared
{
  public class ProcessStabilityRepository : IProcessStabilityRepository
  {
    ...
    {
      DataSet dataSetAsync = await UnitOfWork.GetDataSetAsync();
      return (IEnumerable<ProcessStability>) (await dataSetAsync.SharedProcessStabilities.ToListAsync<ProcessStability>()).OrderByDescending<ProcessStability, Decimal>((Func<ProcessStability, Decimal>) (m => m.Weighting)).ThenBy<ProcessStability, string>((Func<ProcessStability, string>) (m => m.Name));
    }
