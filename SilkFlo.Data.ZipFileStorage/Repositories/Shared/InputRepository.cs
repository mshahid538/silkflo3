...
namespace SilkFlo.Data.Persistence.Repositories.Shared
{
  public class InputRepository : IInputRepository
  {
    ...
    {
      DataSet dataSetAsync = await UnitOfWork.GetDataSetAsync();
      return (IEnumerable<Input>) (await dataSetAsync.SharedInputs.ToListAsync<Input>()).OrderByDescending<Input, Decimal>((Func<Input, Decimal>) (m => m.Weighting)).ThenBy<Input, string>((Func<Input, string>) (m => m.Name));
    }
