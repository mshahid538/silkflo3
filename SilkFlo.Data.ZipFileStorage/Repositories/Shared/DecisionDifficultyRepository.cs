...
namespace SilkFlo.Data.Persistence.Repositories.Shared
{
  public class DecisionDifficultyRepository : IDecisionDifficultyRepository
  {
    ...
    {
      DataSet dataSetAsync = await UnitOfWork.GetDataSetAsync();
      return (IEnumerable<DecisionDifficulty>) (await dataSetAsync.SharedDecisionDifficulties.ToListAsync<DecisionDifficulty>()).OrderByDescending<DecisionDifficulty, Decimal>((Func<DecisionDifficulty, Decimal>) (m => m.Weighting)).ThenBy<DecisionDifficulty, string>((Func<DecisionDifficulty, string>) (m => m.Name));
    }
