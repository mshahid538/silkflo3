...
namespace SilkFlo.Data.Persistence.Repositories.Shared
{
  public class StageGroupRepository : IStageGroupRepository
  {
    ...
    {
      DataSet dataSetAsync = await UnitOfWork.GetDataSetAsync();
      return (IEnumerable<StageGroup>) (await dataSetAsync.SharedStageGroups.ToListAsync<StageGroup>()).OrderBy<StageGroup, int>((Func<StageGroup, int>) (m => m.Sort)).ThenBy<StageGroup, string>((Func<StageGroup, string>) (m => m.Name));
    }
