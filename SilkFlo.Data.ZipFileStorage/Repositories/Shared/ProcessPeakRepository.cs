...
namespace SilkFlo.Data.Persistence.Repositories.Shared
{
  public class ProcessPeakRepository : IProcessPeakRepository
  {
    ...
    {
      DataSet dataSetAsync = await UnitOfWork.GetDataSetAsync();
      return (IEnumerable<ProcessPeak>) (await dataSetAsync.SharedProcessPeaks.ToListAsync<ProcessPeak>()).OrderByDescending<ProcessPeak, Decimal>((Func<ProcessPeak, Decimal>) (m => m.Weighting)).ThenBy<ProcessPeak, string>((Func<ProcessPeak, string>) (m => m.Name));
    }
