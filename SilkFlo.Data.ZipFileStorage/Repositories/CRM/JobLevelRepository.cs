...
namespace SilkFlo.Data.Persistence.Repositories.CRM
{
  public class JobLevelRepository : IJobLevelRepository
  {
    ...
    {
      DataSet dataSetAsync = await UnitOfWork.GetDataSetAsync();
      return (IEnumerable<JobLevel>) (await dataSetAsync.CRMJobLevels.ToListAsync<JobLevel>()).OrderBy<JobLevel, int>((Func<JobLevel, int>) (m => m.Sort)).ThenBy<JobLevel, string>((Func<JobLevel, string>) (m => m.Name));
    }
