...
namespace SilkFlo.Data.Persistence.Repositories.CRM
{
  public class CompanySizeRepository : ICompanySizeRepository
  {
    ...
    {
      DataSet dataSetAsync = await UnitOfWork.GetDataSetAsync();
      return (IEnumerable<CompanySize>) (await dataSetAsync.CRMCompanySizes.ToListAsync<CompanySize>()).OrderBy<CompanySize, int>((Func<CompanySize, int>) (m => m.Sort)).ThenBy<CompanySize, string>((Func<CompanySize, string>) (m => m.Name));
    }
