...
namespace SilkFlo.Data.Persistence.Repositories.CRM
{
  public class ProspectRepository : IProspectRepository
  {
    ...
    {
      DataSet dataSetAsync = await UnitOfWork.GetDataSetAsync();
      return (IEnumerable<Prospect>) (await dataSetAsync.CRMProspects.ToListAsync<Prospect>()).OrderBy<Prospect, string>((Func<Prospect, string>) (m => m.CompanyName)).ThenBy<Prospect, string>((Func<Prospect, string>) (m => m.FirstName)).ThenBy<Prospect, string>((Func<Prospect, string>) (m => m.LastName));
    }
